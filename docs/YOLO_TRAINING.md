...
| 環節 | 實測/概算 |
|---|---|
| JPEG 編碼 + 傳輸（790x520 縮到 640 寬，約 89KB @ q80） | 區網 ~3ms；WiFi/VPN 20~150ms |
| 伺服器端 `model.predict()`（含前後處理，5090 上實測） | **~18ms** |
| **合計往返** | 區網 **20~30ms**；VPN 視頻寬而定 |

> 注意：常聽到的「YOLO 只要 1~3ms」指的是純 GPU 推理；透過 ultralytics 的
> `predict()` 還有前處理、NMS、結果封裝的 Python 開銷，實際約 18ms。
> 對 125ms 的預算來說仍然充裕。
