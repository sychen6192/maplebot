...
def run_test(image, prompt_map, out_path):
    predict, classes = load_teacher(prompt_map)
    dets = predict(image)
    print(f"在 {image} 偵測到 {len(dets)} 個框（類別 {classes}）")
    ...
    print(f"標註預覽已存到 {out_path}")
    if not dets:
        print("⚠ 一個都沒框到：GroundingDINO 可能認不得這種 sprite。"
              "換個 prompt 再試，或改用模板老師 tools/autolabel.py")
